...
public class GamePanel extends JPanel {
				...
				if(SwingUtilities.isRightMouseButton(e)) {
					...
					if(!moveInProgress) {
						if(!activeGameTeam.contains(hex.getGameChar())) {
							map.focus(hex);
							map.getHex(new Hex(0, 0, 0)).setHighlighted(true);
						}
						else {
							...
							for(Hex h : moveableTiles.keySet()) {
	...
	public void endMove() {
		...
		map.focus(null);
		map.highlightHexes(map.getMap(), false);
	}
	...
	public void setInfoPanelList(JList<GameChar> infoPanelList) {
		...
		listModel = new DefaultListModel<GameChar>();
		System.out.println(infoPanelList);
	...
	public void selectCharacter(GameChar c) {
		...
		map.highlightHexes(moveableTiles.keySet(), true);
		map.highlightHexes(attackableTiles.keySet(), true);
	...
	public Map<Hex, Integer> getMoveableTiles(GameChar c) {
		...
		for(int i = 0; i < c.getMoves(); i++) { //i is the number of "steps" the character will have to take
		...
		for(Map.Entry<Hex, Integer> entry : moveableTiles.entrySet()) {
			for (Hex n : entry.getKey().getNeighbors()) {
				if(!(n.getGameChar() == null) && !activeGameTeam.contains(n.getGameChar())) {
					if(!enemyTiles.containsKey(n)) enemyTiles.put(n, entry);
				}
			}
		}
		return enemyTiles;
