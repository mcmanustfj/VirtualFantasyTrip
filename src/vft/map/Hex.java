...
public class Hex {
    ...
    public void paint(Graphics2D g, Layout l) {
        if(focused){
            ...
            if(isOccupied()) {
                ...
                g.setColor(new Color(cColor.getRed(), cColor.getGreen(), cColor.getBlue(), 50));
            }
            g.fill(toPoly(l));
            g.setColor(Color.black);
    ...
    public Color getColor() {
        if(character == null)  {   
            ...
            if(character.getColor() == null) return Color.green;
            else if(highlighted) return highlightColor;
